defmodule MobiusSmarts.Recipes.CoreNx do
  @moduledoc """
  Recipes you can run on Mobius data using only Nx. Each function is
  framed around a question you might ask of the kinds of metrics a
  Nerves device tends to emit; the data names are illustrative.

  All functions take an already-pulled tensor. See
  `MobiusSmarts.Source.series/4` for pulling a metric's values out
  of Mobius as a tensor.
  ...
  When you carry a validity mask alongside a series (a `{n}` u8 tensor,
  `1` = present), use `mean_with_validity/2` to keep gaps from skewing
  the answer.
  """
  ...
  ## Examples
      ...
      iex> MobiusSmarts.Recipes.CoreNx.ran_hot?(temps, window: 2, threshold: 70.0)
      true
  ...
  def ran_hot?(temps, opts \\ []) do
    # Clamp so a series shorter than the window (default is sized for
    # 1 Hz samples) degrades to a plain max instead of raising.
    window = opts |> Keyword.get(:window, 300) |> min(Nx.size(temps)) |> max(1)
  ...
  ## Examples
      ...
      iex> slope = MobiusSmarts.Recipes.CoreNx.mem_slope_per_day(mem)
  ...
  ## Examples
      ...
      iex> MobiusSmarts.Recipes.CoreNx.disk_days_to_threshold(disk, 95.0)
      ...> |> Float.round(1)
      5.0

      iex> declining = Nx.tensor([60.0, 55.0, 50.0, 45.0])
      iex> MobiusSmarts.Recipes.CoreNx.disk_days_to_threshold(declining, 95.0)
      :infinity
  """
  ...
  Returns `nil` when no rate is computable: fewer than 2 samples, or
  every diff negative (e.g. a single reset between two samples).

  ## Examples

      iex> counter = Nx.tensor([10, 20, 30], type: :s64)
      iex> MobiusSmarts.Recipes.CoreNx.mean_rate(counter) |> Float.round(2)
      10.0

      iex> # A reset between sample 2 and 3 — negative diff is masked
      iex> counter_with_reset = Nx.tensor([10, 20, 5, 25], type: :s64)
      iex> MobiusSmarts.Recipes.CoreNx.mean_rate(counter_with_reset)
      ...> |> Float.round(2)
      15.0

      iex> MobiusSmarts.Recipes.CoreNx.mean_rate(Nx.tensor([10, 4], type: :s64))
      nil
  """
  @spec mean_rate(Nx.Tensor.t()) :: float() | nil
  def mean_rate(counter) do
    if Nx.size(counter) < 2 do
      nil
    else
      rate = counter |> Nx.diff() |> Nx.as_type(:f32)
      valid = Nx.greater_equal(rate, 0)
      n_valid = valid |> Nx.sum() |> Nx.to_number()

      if n_valid == 0 do
        nil
      else
        clean = Nx.select(valid, rate, 0.0)
        Nx.to_number(Nx.sum(clean)) / n_valid
      end
    end
  end
  ...
  ## Examples
      ...
      iex> hist = MobiusSmarts.Recipes.CoreNx.value_histogram(temps, 20.0, 5.0, 4)
  ...
  ## Examples
      ...
      iex> z = MobiusSmarts.Recipes.CoreNx.z_score(window, 55.0)
      iex> Float.round(z, 2)
      7.07
  """
  ...
  def z_score(window, latest) do
    ...
    if sigma == 0.0 do
      raise ArgumentError,
            "window has zero variance — a z-score against a constant window is undefined"
    end
  ...
  ## Examples
      ...
      iex> window = Nx.tensor([50.0, 51.0, 49.0, 50.5, 49.5])
      iex> MobiusSmarts.Recipes.CoreNx.anomalous?(window, 55.0)
      true
      iex> MobiusSmarts.Recipes.CoreNx.anomalous?(window, 50.2)
      false
  """
  ...
  ## Examples
      ...
      iex> rolled = MobiusSmarts.Recipes.CoreNx.rolling_means(values, [2, 3])
  ...
  `validity` is a `{n}` u8 tensor of `1` (present) / `0` (missing).

  ## Examples

      iex> values = Nx.tensor([10.0, 20.0, 30.0, 40.0])
      iex> validity = Nx.tensor([1, 0, 1, 1], type: :u8)
      iex> MobiusSmarts.Recipes.CoreNx.mean_with_validity(values, validity)
      ...> |> Float.round(2)
      26.67
  """
  ...
  ## Examples
      ...
      iex> MobiusSmarts.Recipes.CoreNx.quantile(t, 0.0)
      10.0
      iex> MobiusSmarts.Recipes.CoreNx.quantile(t, 1.0)
      50.0
      iex> MobiusSmarts.Recipes.CoreNx.quantile(t, 0.5)
      30.0
  """
